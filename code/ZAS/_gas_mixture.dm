...
#define HEAT_CAPACITY_CALCULATION(oxygen,carbon_dioxide,nitrogen,plasma) \
	max(0, carbon_dioxide * SPECIFIC_HEAT_CDO + (oxygen + nitrogen) * SPECIFIC_HEAT_AIR + plasma * SPECIFIC_HEAT_TOXIN)
...
/datum/gas_mixture/proc/heat_capacity()
	...
	var/heat_capacity = HEAT_CAPACITY_CALCULATION(oxygen,carbon_dioxide,nitrogen,plasma)
...
/datum/gas_mixture/proc/heat_capacity_archived()
	...
	var/heat_capacity_archived = HEAT_CAPACITY_CALCULATION(oxygen_archived,carbon_dioxide_archived,nitrogen_archived,plasma_archived)
...
/datum/gas_mixture/proc/total_moles()
	return total_moles
	/*var/moles = oxygen + carbon_dioxide + nitrogen + plasma
...
/datum/gas_mixture/proc/update_values()
	...
	total_moles = oxygen + carbon_dioxide + nitrogen + plasma
...
/datum/gas_mixture/proc/check_tile_graphic()
	...
	graphic = 0
	if(plasma > MOLES_PLASMA_VISIBLE)
		graphic = 1
...
/datum/gas_mixture/proc/check_then_merge(datum/gas_mixture/giver)
	...
	if(((giver.oxygen > MINIMUM_AIR_TO_SUSPEND) && (giver.oxygen >= oxygen*MINIMUM_AIR_RATIO_TO_SUSPEND)) \
		...
		|| ((giver.plasma > MINIMUM_AIR_TO_SUSPEND) && (giver.plasma >= plasma*MINIMUM_AIR_RATIO_TO_SUSPEND)))
		return 0
...
/datum/gas_mixture/proc/merge(datum/gas_mixture/giver)
	...
	if((group_multiplier>1)||(giver.group_multiplier>1))
		...
		plasma += giver.plasma*giver.group_multiplier/group_multiplier
	else
		oxygen += giver.oxygen
...
/datum/gas_mixture/proc/remove(amount)
	...
	removed.plasma = QUANTIZE(((plasma/sum)*amount))
	...
	plasma -= removed.plasma/group_multiplier
...
/datum/gas_mixture/proc/remove_ratio(ratio)
	...
	removed.plasma = QUANTIZE(plasma*ratio)
...
/datum/gas_mixture/proc/check_gas_mixture(datum/gas_mixture/sharer)
	...
	var/delta_plasma = QUANTIZE(plasma_archived - sharer.plasma_archived)/TRANSFER_FRACTION
	...
	if(((abs(delta_oxygen) > MINIMUM_AIR_TO_SUSPEND) && (abs(delta_oxygen) >= oxygen_archived*MINIMUM_AIR_RATIO_TO_SUSPEND)) \
		...
		|| ((abs(delta_plasma) > MINIMUM_AIR_TO_SUSPEND) && (abs(delta_plasma) >= plasma_archived*MINIMUM_AIR_RATIO_TO_SUSPEND)))
		return 0
	...
	if(((abs(delta_oxygen) > MINIMUM_AIR_TO_SUSPEND) && (abs(delta_oxygen) >= sharer.oxygen_archived*MINIMUM_AIR_RATIO_TO_SUSPEND)) \
		...
		|| ((abs(delta_plasma) > MINIMUM_AIR_TO_SUSPEND) && (abs(delta_plasma) >= sharer.plasma_archived*MINIMUM_AIR_RATIO_TO_SUSPEND)))
		return -1
...
/datum/gas_mixture/proc/check_turf(turf/model)
	...
	var/delta_plasma = (plasma_archived - model.plasma)/TRANSFER_FRACTION
...
/datum/gas_mixture/proc/share(datum/gas_mixture/sharer)
	...
	if(abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
		...
		if(delta_plasma)
			var/plasma_heat_capacity = SPECIFIC_HEAT_TOXIN*delta_plasma
			if(delta_plasma > 0)
				heat_self_to_sharer += plasma_heat_capacity*temperature_archived
				heat_capacity_self_to_sharer += plasma_heat_capacity
			else
				heat_sharer_to_self -= plasma_heat_capacity*sharer.temperature_archived
				heat_capacity_sharer_to_self -= plasma_heat_capacity
	...
	plasma -= delta_plasma/group_multiplier
	sharer.plasma += delta_plasma/sharer.group_multiplier

	var/moved_moles = (delta_oxygen + delta_carbon_dioxide + delta_nitrogen + delta_plasma)
...
/datum/gas_mixture/proc/mimic(turf/model, border_multiplier)
	...
	var/delta_plasma = QUANTIZE(plasma_archived - model.plasma)/TRANSFER_FRACTION
	...
	if(abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
		...
		if(delta_plasma)
			var/plasma_heat_capacity = SPECIFIC_HEAT_TOXIN*delta_plasma
			heat_transferred -= plasma_heat_capacity*model.temperature
			heat_capacity_transferred -= plasma_heat_capacity
	...
	if(border_multiplier)
		...
		plasma -= delta_plasma*border_multiplier/group_multiplier
	else
		oxygen -= delta_oxygen/group_multiplier
		carbon_dioxide -= delta_carbon_dioxide/group_multiplier
		nitrogen -= delta_nitrogen/group_multiplier
	...
	if((delta_temperature > MINIMUM_TEMPERATURE_TO_MOVE) || abs(moved_moles) > MINIMUM_MOLES_DELTA_TO_MOVE)
		var/delta_pressure = temperature_archived*(total_moles() + moved_moles) - model.temperature*(model.oxygen+model.carbon_dioxide+model.nitrogen+model.plasma)
		return delta_pressure*R_IDEAL_GAS_EQUATION/volume
	else
		return 0
...
/datum/gas_mixture/proc/compare(datum/gas_mixture/sample)
	...
	if((abs(carbon_dioxide-sample.carbon_dioxide) > MINIMUM_AIR_TO_SUSPEND) && \
		...
		return 0
	if((abs(plasma-sample.plasma) > MINIMUM_AIR_TO_SUSPEND) && \
		((plasma < (1-MINIMUM_AIR_RATIO_TO_SUSPEND)*sample.plasma) || (plasma > (1+MINIMUM_AIR_RATIO_TO_SUSPEND)*sample.plasma)))
		return 0
...
/datum/gas_mixture/proc/subtract(datum/gas_mixture/right_side)
	...
	plasma = max(plasma - right_side.plasma)
