...
/turf/simulated/wall/relativewall()
	...
	if(istype(src,/turf/simulated/wall/stonemanual))
		return
	...
	for(var/turf/simulated/wall/W in orange(src,1))
		if(abs(src.x-W.x)-abs(src.y-W.y) && istype(W, src.type) || abs(src.x-W.x)-abs(src.y-W.y) && istype(W, /turf/simulated/wall/stonenoopacity)) //doesn't count diagonal walls
