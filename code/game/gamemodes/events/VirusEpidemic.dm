			...
			for(var/mob/living/carbon/human/H in world)
				...
				if(prob(100))	// no lethal diseases outside virus mode!
					...
					if(prob(20))//don't want people to know that the virus alert = greater virus
						command_alert("Probable outbreak of level [rand(1,6)] viral biohazard aboard [vessel_name()]. All personnel must contain the outbreak.", "Virus Alert")
				else
					infect_mob_random_greater(H)
					if(prob(80))
						command_alert("Probable outbreak of level [rand(2,9)] viral biohazard aboard [vessel_name()]. All personnel must contain the outbreak.", "Virus Alert")
				break
