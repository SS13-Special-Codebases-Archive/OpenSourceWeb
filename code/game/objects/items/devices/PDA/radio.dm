/obj/item/radio/integrated
	...
	proc/post_signal(var/freq, var/key, var/value, var/key2, var/value2, var/key3, var/value3, s_d_filter)
		...
		frequency.post_signal(src, signal, d_filter = s_d_filter)
	...
	New()
		...
		spawn(5)
			if(radio_controller)
				...
				spawn(10)
					post_signal(beacon_freq, "findbeacon", "delivery", s_d_filter = RADIO_NAVBEACONS)
	...
	Topic(href, href_list)
		...
		switch(href_list["op"])
			...
			if("control")
				...
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)

			if("scanbots")		// find all bots
				botlist = null
				post_signal(control_freq, "command", "bot_status", s_d_filter = RADIO_MULEBOT)
			...
			if("unload")
				post_signal(control_freq, cmd, "unload", s_d_filter = RADIO_MULEBOT)
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)
			if("setdest")
				if(beacons)
					var/dest = input("Select Bot Destination", "Mulebot [active.suffix] Interlink", active.destination) as null|anything in beacons
					if(dest)
						post_signal(control_freq, cmd, "target", "destination", dest, s_d_filter = RADIO_MULEBOT)
						post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)

			if("retoff")
				post_signal(control_freq, cmd, "autoret", "value", 0, s_d_filter = RADIO_MULEBOT)
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)
			if("reton")
				post_signal(control_freq, cmd, "autoret", "value", 1, s_d_filter = RADIO_MULEBOT)
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)

			if("pickoff")
				post_signal(control_freq, cmd, "autopick", "value", 0, s_d_filter = RADIO_MULEBOT)
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)
			if("pickon")
				post_signal(control_freq, cmd, "autopick", "value", 1, s_d_filter = RADIO_MULEBOT)
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)

			if("stop", "go", "home")
				post_signal(control_freq, cmd, href_list["op"], s_d_filter = RADIO_MULEBOT)
				post_signal(control_freq, cmd, "bot_status", s_d_filter = RADIO_MULEBOT)
