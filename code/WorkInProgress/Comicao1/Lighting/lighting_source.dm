...
/datum/light_source/New(var/atom/owner, var/atom/top)
	...
	light_typo = source_atom.light_typo
...
/datum/light_source/proc/check()
	...
	if(source_atom.light_typo != light_typo)
		light_typo = source_atom.light_typo
		. = 1
...
/datum/light_source/proc/apply_lum()
	...
	FOR_DVIEW(var/turf/T, light_outer_range, source_turf, INVISIBILITY_LIGHTING)
		...
		if(istype(T, /turf/simulated/floor/open))
			var/turf/simulated/floor/open/O = T
			T = O.floorbelow
			goto check_t
...
/datum/light_source/proc/smart_vis_update()
	...
	FOR_DVIEW(var/turf/T, light_outer_range, source_turf, 0)
		...
		var/turf/simulated/floor/open/O = T
		if(istype(O) && O.floorbelow)
			// Consider the turf below us as well. (Z-lights)
			for(T = O.floorbelow; !isnull(T); T = update_the_turf(T,corners, turfs));
	END_FOR_DVIEW
