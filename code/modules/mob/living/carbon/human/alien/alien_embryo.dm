...
/obj/item/alien_embryo/process()
	...
	switch(stage)
		if(2, 3)
			...
			if(prob(1))
				to_chat(affected_mob, "<span class='combatbold'>I feel bad.</span>")
			if(prob(1))
				to_chat(affected_mob, "<span class='combatbold'>I feel abnormal.</span>")
		if(4)
			if(prob(1))
				affected_mob.emote("sneeze")
			if(prob(1))
				affected_mob.emote("cough")
			if(prob(2))
				to_chat(affected_mob, "<span class='combatbold'>My muscles ache a lot.</span>")
				if(prob(20))
					affected_mob.take_organ_damage(1)
			if(prob(2))
				to_chat(affected_mob, "<span class='combatbold'>My stomach hurts a lot.</span>")
				if(prob(20))
					affected_mob.adjustToxLoss(1)
					affected_mob.updatehealth()
		if(5)
			to_chat(affected_mob, "<span class='combatbold'>SOMETHING ITS TEARING ITS WAY OUT OF MY STOMACH!</span>")
...
/obj/item/alien_embryo/proc/AttemptGrow(var/gib_on_success = 1)
	...
	spawn(6)
		...
		new_xeno.key = picked
		new_xeno.mind.key = picked
		var/area/A = get_area(new_xeno)
		A.Entered(new_xeno)
		playsound(new_xeno.loc, 'sound/voice/hiss5.ogg',100)	//To get the player's attention
		if(gib_on_success)
			affected_mob.gib()
		qdel(src)
		to_chat(new_xeno, "I am now a worm. I must wait until my evolution.")
		to_chat(new_xeno, "WISH #1: CREATE 6 OF OUR OWN.")
